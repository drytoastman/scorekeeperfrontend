...
public class SerialPortUtilTest
{
    ...
    public void multipleNewlineTest() throws Exception
    {
        ...
        buffer.appendData(new byte[] { 51, 52, 13, 54, 55, 10, 56, 57, 13, 10, 58, 59, 10, 13, 99 });
    ...
    public void singleInputsTest() throws Exception
    {
        ...
        buffer.appendData(new byte[] { 51 });
        Assert.assertArrayEquals(null, buffer.getNextLine());
        buffer.appendData(new byte[] { 52 });
        Assert.assertArrayEquals(null, buffer.getNextLine());

        buffer.appendData(new byte[] { 13 });
        Assert.assertArrayEquals(new byte[] { 51, 52 }, buffer.getNextLine());

        buffer.appendData(new byte[] { 10 });
        Assert.assertArrayEquals(null, buffer.getNextLine());

        buffer.appendData(new byte[] { 53 });
        Assert.assertArrayEquals(null, buffer.getNextLine());
        buffer.appendData(new byte[] { 54 });
        Assert.assertArrayEquals(null, buffer.getNextLine());
