...
import org.wwscc.system.docker.models.Runtime;
import org.wwscc.system.docker.models.SwarmInfo;
import org.wwscc.system.docker.models.SystemInfoDefaultAddressPools;
...
public class SystemInfo {
  ...
  private Boolean pidsLimit = null;
  ...
  **/
   ...
   * @return osType
  **/
  @ApiModelProperty(example = "linux", value = "Generic type of the operating system of the host, as returned by the Go runtime (`GOOS`).  Currently returned values are \"linux\" and \"windows\". A full list of possible values can be found in the [Go documentation](https://go.dev/doc/install/source#environment). ")
   ...
   * @return architecture
  **/
  @ApiModelProperty(example = "x86_64", value = "Hardware architecture of the host, as returned by the Go runtime (`GOARCH`).  A full list of possible values can be found in the [Go documentation](https://go.dev/doc/install/source#environment). ")
  ...
  **/
  @ApiModelProperty(example = "[\"name=apparmor\",\"name=seccomp,profile=default\",\"name=selinux\",\"name=userns\",\"name=rootless\"]", value = "List of security features that are enabled on the daemon, such as apparmor, seccomp, SELinux, user-namespaces (userns), rootless and no-new-privileges.  Additional configuration options for each security feature may be present, and are included as a comma-separated list of key/value pairs. ")
  ...
  **/
   ...
   * @return warnings
  **/
  @ApiModelProperty(example = "[\"WARNING: No memory limit support\"]", value = "List of warnings / informational messages about missing features, or issues related to the daemon configuration.  These messages can be printed by the client as information to the user. ")
  ...
  public SystemInfo cdISpecDirs(List<String> cdISpecDirs) {
    this.cdISpecDirs = cdISpecDirs;
    return this;
  }

  public SystemInfo addCdISpecDirsItem(String cdISpecDirsItem) {
    if (this.cdISpecDirs == null) {
      this.cdISpecDirs = new ArrayList<>();
    }
    this.cdISpecDirs.add(cdISpecDirsItem);
    return this;
  }
   ...
   * @return cdISpecDirs
  **/
  @ApiModelProperty(example = "[\"/etc/cdi\",\"/var/run/cdi\"]", value = "List of directories where (Container Device Interface) CDI specifications are located.  These specifications define vendor-specific modifications to an OCI runtime specification for a container being created.  An empty list indicates that CDI device injection is disabled.  Note that since using CDI device injection requires the daemon to have experimental enabled. For non-experimental daemons an empty list will always be returned. ")
  public List<String> getCdISpecDirs() {
    return cdISpecDirs;
  }

  public void setCdISpecDirs(List<String> cdISpecDirs) {
    this.cdISpecDirs = cdISpecDirs;
  }
