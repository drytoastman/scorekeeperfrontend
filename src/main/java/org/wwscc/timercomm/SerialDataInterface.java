...
public class SerialDataInterface implements SerialPortEventListener
{
	...
	class ByteBuffer
	{
		...
		public byte[] getNextLine()
		{
			...
			{
				if ((buf[search] == '\r') && (buf[search+1] == '\n'))
				{
					byte[] ret = new byte[search];
					System.arraycopy(buf, 0, ret, 0, search); // copy out line from start of buffer
					search += 2; // skip the \r\n
					System.arraycopy(buf, search, buf, 0, count-search); // move from search back to start of buffer
					count -= search;
					search = 0;
					return ret;
				}
